...
> **Status: v1.1.1-beta.** Pre-production engine backed by native SpiderMonkey through the `mozjs` crate. SpiderMonkey's own `jit` crate feature is enabled by default, so hot fetch handlers can tier up in the native runtime instead of executing inside a Wasm sandbox without executable pages.
